...
void output_fft_spectrum(void)
{
    ...
    my_printf(&huart3, "=== FFT Spectrum Analysis ===\r\n");
    my_printf(&huart3, "Sampling Freq: %.0f Hz\r\n", sampling_freq);
    my_printf(&huart3, "Freq Resolution: %.2f Hz\r\n", freq_resolution);
    my_printf(&huart3, "--- Spectrum Data ---\r\n");

    // 打印部分频谱数据（从第10个频点开始，以避开直流和低频噪声）
    for (i = 10; i < FFT_LENGTH / 2; i++)
    {
        current_freq = i * freq_resolution;
        my_printf(&huart3, "%.1f Hz: %.6f\r\n", current_freq, fft_magnitude[i]);
    }

    my_printf(&huart3, "--- Peak Analysis ---\r\n");
    ...
    my_printf(&huart3, "Peak Freq: %.0f Hz (Raw: %.2f Hz), Magnitude: %.6fv\r\n", peak_freq, peak_freq_raw, max_magnitude);
    my_printf(&huart3, "THD: %.3f%%\r\n", thd);
    my_printf(&huart3, "THD+N: %.3f%%\r\n", thd_n);
    my_printf(&huart3, "SINAD: %.1f dB\r\n", sinad);
    my_printf(&huart3, "DC Component: %.6fv\r\n", fft_magnitude[0]);
    my_printf(&huart3, "=== End of Spectrum ===\r\n");
}
